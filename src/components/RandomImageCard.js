...
const RandomCard = ({style, item, onPressCard}) => {
    ...
    return (<>
        ...
        <TouchableOpacity activeOpacity={0.9} onPress={onPressCard} zIndex={0}>
            ...
            <FastImage
                key={item.id}
                style={[{flex: 1, margin:2, borderRadius: wp(0),height:hp(24),width:hp(24)}, style]}
                source={{
                    uri: image,
                }}
                resizeMode="cover"
            ...
            >
                <LinearGradient
                    ...
                    style={{
                        borderRadius: wp(0), width: hp(24), height: randomBool ? hp(24) : hp(24), justifyContent: 'flex-end',
                    }}
                    start={{x: 0.0, y: 0.0}}
                    end={{x: 0.0, y: 1.0}}
                >
