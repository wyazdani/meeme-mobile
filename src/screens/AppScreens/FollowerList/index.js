...
const FollowerList = ({navigation}) => {
    ...
    const getAllFollowers = async (size) => {
        setIsLoading({...isLoading, mainLoading: true});
        let res = await HitApi(`${FOLLWER_LIST + isSize}&key=followers`, 'get', '', token,);

        if (res?.status == 200) {
            if (res?.data?.followers?.length != 0) {
                if (size == 1) {
                    setIsList({
                        permList: res?.data?.followers, tempList: res?.data?.followers,
                    });
                } else {
                    setIsList({
                        permList: [...isList.permList, ...res?.data?.followers],
                        tempList: [...isList.tempList, ...res?.data?.followers],
                    });
                }
    ...
    const renderItem = ({item}) => {
        return (<FollList item={item} onPressUserProfile={() => followerPress(item?.follower_user_detail?.id)}/>);
    };

    const onChangeText = (text) => {
        setIsSearch(text);
        let t = text?.toLowerCase();
        let obj = isList?.permList?.filter((i) => i?.follower_user_detail?.username?.toLowerCase()?.indexOf(t) > -1,);
